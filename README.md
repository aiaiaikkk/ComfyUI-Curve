...
#### 🎨 PS Curve预设系统完善
- ✅ 修复预设管理器界面，与其他节点保持一致的样式
- ✅ 解决预设管理器z-index显示层级问题
- ✅ 修复默认预设（人像增强、电影风格、风景鲜艳）加载失败问题
- ✅ 支持分号分隔的曲线数据格式
- ✅ 扩大mask_blur范围至0-50，提供更细腻的羽化控制
- ✅ 移除不必要的use_modal开关，统一使用双击打开弹窗

#### 🎯 HSL预设系统修复
- ✅ 修复预设API返回格式错误
- ✅ 实现完整的预设管理功能（保存、加载、删除、导出、导入）
- ✅ 统一预设管理器界面风格

#### 🔧 遮罩功能修复 (2025-06-23)
- ✅ 解决所有节点遮罩功能失效的关键问题
- ✅ 修复前端预览正确但后端返回原图的bug
- ✅ 确保有遮罩时即使参数为默认值也会正确处理
...
#### 2025-06-23 - 关键问题修复
- **遮罩功能修复**：
  - 修复了所有节点（PS Curve、Color Grading、PS Levels、Camera Raw、PS HSL）的遮罩功能失效问题
  - 解决了前端预览正确但后端返回原图的bug
  - 优化了BaseImageNode基类，确保有遮罩时即使参数为默认值也会正确处理

#### 2025-06-22 - PS Curve节点预设系统
- **预设系统完成**：
  - 支持保存和加载曲线预设
  - 预设分类管理（默认/用户/共享）
  - 预设导入/导出功能
